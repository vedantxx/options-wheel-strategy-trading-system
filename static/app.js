...
function renderWheelChart(){const p=state.position;if(!p||!window.Chart)return;if(state.charts.wheel)state.charts.wheel.destroy();const current=Number(p.spot),costBasis=Number(p.average_cost),filledCall=(p.short_legs||[]).find(l=>l.type==='call'),filledPut=(p.short_legs||[]).find(l=>l.type==='put'),pending=state.wheel?.pending_orders||[];const moveFromBasis=costBasis?((current-costBasis)/costBasis):0;const inOrangeBand=Math.abs(moveFromBasis)<=.005;const barColor=inOrangeBand?'#f5ad5d':moveFromBasis>.005?'#23c98b':'#f26374';const barFill=inOrangeBand?'rgba(245,173,93,.60)':moveFromBasis>.005?'rgba(35,201,139,.58)':'rgba(242,99,116,.58)';const levels=[current,costBasis,state.selectedCall?.strike,state.selectedPut?.strike,filledCall?.strike,filledPut?.strike,...pending.map(order=>order.strike)].filter(v=>Number.isFinite(Number(v))).map(Number);const scaleFloor=Math.min(...levels)*.90;const scaleCeiling=Math.max(...levels)*1.08;state.charts.wheel=new Chart($('wheelChart'),{type:'bar',data:{labels:['Current price'],datasets:[{data:[current],backgroundColor:barFill,borderColor:barColor,borderWidth:1,borderRadius:7,barThickness:72}]},options:{...chartDefaults(),plugins:{...chartDefaults().plugins,wheelLines:{cost:costBasis,call:state.selectedCall?.strike,put:state.selectedPut?.strike,filledCall:filledCall?.strike,filledCallPremium:Math.abs(Number(filledCall?.avg_entry_price||0)),filledPut:filledPut?.strike,filledPutPremium:Math.abs(Number(filledPut?.avg_entry_price||0)),pending},tooltip:{callbacks:{label:()=>`Current ${money(current,2)} · ${moveFromBasis>=0?'+':''}${(moveFromBasis*100).toFixed(2)}% vs cost basis`}}},scales:{x:{grid:{display:false},ticks:{color:'#8c9aac'}},y:{min:scaleFloor,max:scaleCeiling,grid:{color:'rgba(130,150,175,.10)'},ticks:{color:'#6f7d8e',callback:v=>'$'+Number(v).toFixed(2)}}}},plugins:[wheelLinePlugin]});}
...
function syncButtons(){const contracts=scenario().contracts||1,unlocked=state.tradingLock.unlocked;$('sellCallBtn').disabled=!unlocked||!state.selectedCall;$('sellPutBtn').disabled=!unlocked||!state.selectedPut;const legs=state.position?.short_legs||[],hasCall=legs.some(l=>l.type==='call'),hasPut=legs.some(l=>l.type==='put');$('closeCallBtn').disabled=!unlocked||!hasCall;$('closePutBtn').disabled=!hasPut;const lockHint=unlocked?`${contracts} contract(s)`:'Unlock selling first';$('sellCallBtn').title=lockHint;$('sellPutBtn').title=lockHint;$('closeCallBtn').title=!hasCall?'No open call to close':unlocked?'Close the open call':'Unlock selling first';}
