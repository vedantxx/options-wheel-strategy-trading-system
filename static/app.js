...
const wheelLinePlugin={id:'wheelLines',afterDatasetsDraw(chart,args,opts){const {ctx,chartArea,scales}=chart;if(!opts)return;ctx.save();[['cost','#27c2df','Cost basis',false],['call','#23c98b','Call K',true],['put','#f26374','Put K',true]].forEach(([key,color,label,dotted])=>{if(opts[key]==null)return;const y=scales.y.getPixelForValue(opts[key]);if(y<chartArea.top||y>chartArea.bottom)return;ctx.strokeStyle=color;ctx.setLineDash(dotted?[5,4]:[]);ctx.lineWidth=dotted?1.4:1.5;ctx.beginPath();ctx.moveTo(chartArea.left,y);ctx.lineTo(chartArea.right,y);ctx.stroke();ctx.setLineDash([]);ctx.fillStyle=color;ctx.font='600 9px JetBrains Mono';ctx.fillText(`${label} ${Number(opts[key]).toFixed(2)}`,chartArea.left+7,y-5)});ctx.restore()}};
function renderWheelChart(){const p=state.position;if(!p||!window.Chart)return;if(state.charts.wheel)state.charts.wheel.destroy();const current=Number(p.spot);const costBasis=Number(p.average_cost);const moveFromBasis=costBasis?((current-costBasis)/costBasis):0;const inOrangeBand=Math.abs(moveFromBasis)<=.005;const barColor=inOrangeBand?'#f5ad5d':moveFromBasis>.005?'#23c98b':'#f26374';const barFill=inOrangeBand?'rgba(245,173,93,.60)':moveFromBasis>.005?'rgba(35,201,139,.58)':'rgba(242,99,116,.58)';const levels=[current,costBasis,state.selectedCall?.strike,state.selectedPut?.strike].filter(v=>Number.isFinite(Number(v))).map(Number);const scaleFloor=Math.min(...levels)*.90;const scaleCeiling=Math.max(...levels)*1.08;state.charts.wheel=new Chart($('wheelChart'),{type:'bar',data:{labels:['Current price'],datasets:[{data:[current],backgroundColor:barFill,borderColor:barColor,borderWidth:1,borderRadius:7,barThickness:72}]},options:{...chartDefaults(),plugins:{...chartDefaults().plugins,wheelLines:{cost:costBasis,call:state.selectedCall?.strike,put:state.selectedPut?.strike},tooltip:{callbacks:{label:()=>`Current ${money(current,2)} · ${moveFromBasis>=0?'+':''}${(moveFromBasis*100).toFixed(2)}% vs cost basis`}}},scales:{x:{grid:{display:false},ticks:{color:'#8c9aac'}},y:{min:scaleFloor,max:scaleCeiling,grid:{color:'rgba(130,150,175,.10)'},ticks:{color:'#6f7d8e',callback:v=>'$'+v}}}},plugins:[wheelLinePlugin]});}
