...
const shortDate = (v) => v ? new Date(`${String(v).slice(0,10)}T12:00:00`).toLocaleDateString() : '—';
const tradeState = (trade) => {
  const status = String(trade.status || '').toLowerCase();
  if (status === 'partially_filled') return {label:'Partial', className:'partial'};
  if (['rejected','failed'].includes(status)) return {label:'Rejected', className:'rejected'};
  if (['canceled','expired','done_for_day','replaced','stopped','suspended'].includes(status)) return {label:'Canceled', className:'canceled'};
  if (status && status !== 'filled') return {label:'Pending', className:'pending'};
  const lifecycle = {
    Yes: ['Assigned ITM','assigned-itm'], Expired: ['Expired OTM','expired-otm'],
    No: ['Closed','closed'], Live: ['Live','live'], Open: ['Open','open'], Closed: ['Closed','closed'],
  }[trade.event] || ['Live','live'];
  return {label:lifecycle[0], className:lifecycle[1]};
};
...
function renderTrades(trades) {
  ...
  $('tradesBody').innerHTML = rows.length ? rows.map(t => {
    const assignment = tradeState(t);
    return `<tr><td>${t.time?new Date(t.time).toLocaleDateString():'—'}</td><td><strong>${t.underlying}</strong></td><td><span class="mono">${t.symbol}</span><br><span style="color:var(--muted)">${t.strategy}</span></td><td>${t.side}</td><td class="mono">${num(t.qty,0)}</td><td class="mono">${money(t.price,2)}</td><td><span class="lifecycle ${assignment.className}">${assignment.label}</span></td><td class="mono">${shortDate(t.event_date)}</td><td class="mono ${cls(t.pnl)}">${money(t.pnl,2)}</td><td class="mono ${cls(t.pnl_pct)}">${pct(t.pnl_pct)}</td><td>${t.cancelable&&t.order_id?`<button class="cancel-order-btn" data-order-id="${t.order_id}" data-symbol="${t.symbol}" ${state.tradingLock.unlocked?'':'disabled'}>${state.tradingLock.unlocked?'Cancel order':'🔒 Cancel order'}</button>`:'—'}</td></tr>`;
...
function renderWheel(data) {
  const p=data.position,held=p.held!==false&&Number(p.shares)>0,basisLabel=held?'Cost basis':'Last cost basis',phase=$('wheelPhase'); $('scenarioSymbol').textContent=`${p.symbol} Wheel`; $('wheelChartTitle').textContent=held?`${p.symbol} · ${num(p.shares,0)} shares @ ${money(p.average_cost,2)}`:p.symbol;phase.textContent=held?'↗ CALL':'↘ PUT';phase.className=`phase-pill ${held?'call':'put'}`;
...
function renderChain(type, rows) {
  ...
  if(!rows.length){body.innerHTML='<tr><td colspan="7" class="empty-cell">No OTM contracts returned</td></tr>';return}
  body.innerHTML=rows.map((r,i)=>`<tr data-symbol="${r.symbol}" class="${i===0?'selected':''}"><td>${money(r.strike,2)}</td><td>${money(r.bid,2)}</td><td>${money(r.ask,2)}</td><td>${money(r.mid,2)}</td><td>${r.delta?Number(r.delta).toFixed(4):'—'}</td><td>${r.iv?(r.iv*100).toFixed(1)+'%':'—'}</td><td>${r.volume==null?'—':num(r.volume,0)}</td></tr>`).join('');
...
function renderTradingLock(){const lock=state.tradingLock,button=$('tradeLockBtn'),label=lock.unlocked?'Trading unlocked; click to lock selling':'Trading locked; click to unlock selling',icon=lock.unlocked?button.dataset.unlockIcon:button.dataset.lockIcon;button.classList.toggle('unlocked',lock.unlocked);button.innerHTML=`<img src="${icon}" alt="" aria-hidden="true">`;button.setAttribute('aria-label',label);button.title=!lock.configured?'Set TRADING_UNLOCK_PASSWORD in .env first':lock.unlocked?'Click to lock Sell call, Sell put, Close call, Close put, and Cancel order':'Password required to enable Sell call, Sell put, Close call, Close put, and Cancel order';if(state.lockExpiryTimer){clearTimeout(state.lockExpiryTimer);state.lockExpiryTimer=null}if(lock.unlocked&&lock.expires_at){const delay=Math.max(0,lock.expires_at*1000-Date.now());state.lockExpiryTimer=setTimeout(loadTradingLock,Math.min(delay+250,2147483647))}syncButtons()}
...
async function toggleTradingLock(){if(!state.tradingLock.unlocked){openUnlockModal();return}try{state.tradingLock=await api('/api/trading/lock',{method:'POST',body:'{}'});renderTradingLock();toast('All order actions are locked.')}catch(error){toast(error.message,true)}}
...
function syncButtons(){const contracts=scenario().contracts||1,unlocked=state.tradingLock.unlocked,hasShares=state.position?.held!==false&&Number(state.position?.shares)>=100;$('sellCallBtn').disabled=!unlocked||!state.selectedCall||!hasShares;$('sellPutBtn').disabled=!unlocked||!state.selectedPut;const legs=state.position?.short_legs||[],hasCall=legs.some(l=>l.type==='call'),hasPut=legs.some(l=>l.type==='put');$('closeCallBtn').disabled=!unlocked||!hasCall;$('closePutBtn').disabled=!unlocked||!hasPut;const lockHint=unlocked?`${contracts} contract(s)`:'Unlock selling first';$('sellCallBtn').title=!hasShares?'Covered calls require at least 100 owned shares':lockHint;$('sellPutBtn').title=lockHint;$('closeCallBtn').title=!hasCall?'No open call to close':unlocked?'Close the open call':'Unlock selling first';$('closePutBtn').title=!hasPut?'No open put to close':unlocked?'Close the open put':'Unlock selling first';[...document.querySelectorAll('.cancel-order-btn')].forEach(button=>{button.disabled=!unlocked;button.textContent=unlocked?'Cancel order':'🔒 Cancel order';button.title=unlocked?'Cancel this pending order':'Unlock selling first'});}
...
function clearChains(message){$('callsBody').innerHTML=`<tr><td colspan="7" class="empty-cell">${message}</td></tr>`;$('putsBody').innerHTML=`<tr><td colspan="7" class="empty-cell">${message}</td></tr>`;}
