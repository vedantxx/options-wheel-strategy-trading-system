...
const state = { portfolio: null, wheel: null, position: null, selectedCall: null, selectedPut: null, pendingOrder: null, charts: {} };
...
function syncButtons(){const contracts=scenario().contracts||1;$('sellCallBtn').disabled=!state.selectedCall;$('sellPutBtn').disabled=!state.selectedPut;const legs=state.position?.short_legs||[];$('closeCallBtn').disabled=!legs.some(l=>l.type==='call');$('closePutBtn').disabled=!legs.some(l=>l.type==='put');$('sellCallBtn').title=`${contracts} contract(s)`;$('sellPutBtn').title=`${contracts} contract(s)`;}
...
async function submitOrder(){if(!state.pendingOrder)return;$('confirmOrder').disabled=true;$('confirmOrder').textContent='Submitting…';try{const data=await api('/api/orders',{method:'POST',body:JSON.stringify({...state.pendingOrder,confirm:true})});toast(`${data.paper?'Paper':'Live'} order accepted: ${data.status}`);closeModal();await loadPortfolio(true)}catch(error){toast(error.message,true)}finally{$('confirmOrder').disabled=false;$('confirmOrder').textContent='Submit order'}}
