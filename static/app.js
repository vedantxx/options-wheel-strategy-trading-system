...
const state = { portfolio: null, wheel: null, position: null, selectedCall: null, selectedPut: null, pendingOrder: null, tradingLock: {configured:false,unlocked:false,expires_at:null}, lockExpiryTimer:null, charts: {} };
...
async function loadPortfolio(preserveSymbol=false) {
  ...
  try {
    ...
    if (symbols.length) await loadMaturities(); else clearWheel('No long stock positions found');
...
function renderTrades(trades) {
  ...
  $('tradesBody').innerHTML = rows.length ? rows.map(t => `<tr><td>${t.time?new Date(t.time).toLocaleDateString():'—'}</td><td><strong>${t.underlying}</strong></td><td><span class="mono">${t.symbol}</span><br><span style="color:var(--muted)">${t.strategy}</span></td><td>${t.side}</td><td class="mono">${num(t.qty,0)}</td><td class="mono">${money(t.price,2)}</td><td><span class="status ${t.status}">${t.status}</span></td></tr>`).join('') : '<tr><td colspan="7" class="empty-cell">No matching trades</td></tr>';
}
...
function renderWheel(data) {
  const p=data.position; $('scenarioSymbol').textContent=`${p.symbol} Wheel`; $('wheelChartTitle').textContent=`${p.symbol} · ${num(p.shares,0)} shares @ ${money(p.average_cost,2)}`;
...
function scenario() {
  const p=state.position,c=state.selectedCall,u=state.selectedPut;if(!p)return{};const contracts=Math.max(1,Math.floor(Math.abs(p.shares)/100));
  return {contracts,stockPl:(p.spot-p.average_cost)*p.shares,callPremium:c?c.bid*100*contracts:null,putPremium:u?u.bid*100*contracts:null,calledAway:c?(c.strike-p.average_cost)*100*contracts:null,putCash:u?u.strike*100*contracts:null,putBasis:u?(p.shares*p.average_cost+100*contracts*u.strike)/(p.shares+100*contracts):null};
}

function metric(id,value) { const el=$(id);el.textContent=money(value,2);el.className=cls(value); }
function updateScenario(){const s=scenario(),p=state.position;if(!p)return;metric('stockPl',s.stockPl);metric('callPremium',s.callPremium);metric('putPremium',s.putPremium);metric('calledAway',s.calledAway);$('putCash').textContent=money(s.putCash,2);$('putBasis').textContent=s.putBasis?money(s.putBasis,2):'—';$('stockPlHint').textContent=`${num(p.shares,0)} sh · cost basis ${money(p.average_cost,2)} → current ${money(p.spot,2)} · updates on refresh`;$('callPremiumHint').textContent=state.selectedCall?`${s.contracts} contract(s) · bid × 100`:'Pick a call row';$('putPremiumHint').textContent=state.selectedPut?`${s.contracts} contract(s) · bid × 100`:'Pick a put row';$('calledAwayHint').textContent=state.selectedCall?`${s.contracts} contract(s) @ ${money(state.selectedCall.strike,2)} · ex premium`:'Pick a call row';$('putCashHint').textContent=state.selectedPut?`${s.contracts} contract(s) @ ${money(state.selectedPut.strike,2)}`:'Pick a put row';}

const wheelLinePlugin={id:'wheelLines',afterDatasetsDraw(chart,args,opts){const {ctx,chartArea,scales}=chart;if(!opts)return;ctx.save();[['cost','#27c2df','Cost basis',false,false],['filledCall','#69e1b3','Filled call K',false,true],['filledPut','#ff9daa','Filled put K',false,true],['call','#23c98b','Call scenario K',true,false],['put','#f26374','Put scenario K',true,false]].forEach(([key,color,label,dotted,filled])=>{if(opts[key]==null)return;const y=scales.y.getPixelForValue(opts[key]);if(y<chartArea.top||y>chartArea.bottom)return;ctx.strokeStyle=color;ctx.setLineDash(dotted?[5,4]:[]);ctx.lineWidth=filled?2.4:dotted?1.4:1.5;ctx.beginPath();ctx.moveTo(chartArea.left,y);ctx.lineTo(chartArea.right,y);ctx.stroke();ctx.setLineDash([]);ctx.fillStyle=color;ctx.font='600 9px JetBrains Mono';const premium=filled&&opts[`${key}Premium`]?` · fill $${Number(opts[`${key}Premium`]).toFixed(2)}`:'';const text=`${label} ${Number(opts[key]).toFixed(2)}${premium}`;if(filled){const width=ctx.measureText(text).width;ctx.fillText(text,chartArea.right-width-7,y+12)}else ctx.fillText(text,chartArea.left+7,y-5)});ctx.restore()}};
function renderWheelChart(){const p=state.position;if(!p||!window.Chart)return;if(state.charts.wheel)state.charts.wheel.destroy();const current=Number(p.spot),costBasis=Number(p.average_cost),filledCall=(p.short_legs||[]).find(l=>l.type==='call'),filledPut=(p.short_legs||[]).find(l=>l.type==='put');const moveFromBasis=costBasis?((current-costBasis)/costBasis):0;const inOrangeBand=Math.abs(moveFromBasis)<=.005;const barColor=inOrangeBand?'#f5ad5d':moveFromBasis>.005?'#23c98b':'#f26374';const barFill=inOrangeBand?'rgba(245,173,93,.60)':moveFromBasis>.005?'rgba(35,201,139,.58)':'rgba(242,99,116,.58)';const levels=[current,costBasis,state.selectedCall?.strike,state.selectedPut?.strike,filledCall?.strike,filledPut?.strike].filter(v=>Number.isFinite(Number(v))).map(Number);const scaleFloor=Math.min(...levels)*.90;const scaleCeiling=Math.max(...levels)*1.08;state.charts.wheel=new Chart($('wheelChart'),{type:'bar',data:{labels:['Current price'],datasets:[{data:[current],backgroundColor:barFill,borderColor:barColor,borderWidth:1,borderRadius:7,barThickness:72}]},options:{...chartDefaults(),plugins:{...chartDefaults().plugins,wheelLines:{cost:costBasis,call:state.selectedCall?.strike,put:state.selectedPut?.strike,filledCall:filledCall?.strike,filledCallPremium:Math.abs(Number(filledCall?.avg_entry_price||0)),filledPut:filledPut?.strike,filledPutPremium:Math.abs(Number(filledPut?.avg_entry_price||0))},tooltip:{callbacks:{label:()=>`Current ${money(current,2)} · ${moveFromBasis>=0?'+':''}${(moveFromBasis*100).toFixed(2)}% vs cost basis`}}},scales:{x:{grid:{display:false},ticks:{color:'#8c9aac'}},y:{min:scaleFloor,max:scaleCeiling,grid:{color:'rgba(130,150,175,.10)'},ticks:{color:'#6f7d8e',callback:v=>'$'+Number(v).toFixed(2)}}}},plugins:[wheelLinePlugin]});}
...
function renderTradingLock(){const lock=state.tradingLock,button=$('tradeLockBtn');button.classList.toggle('unlocked',lock.unlocked);button.innerHTML=lock.unlocked?'<span>🔓</span> Lock selling':'<span>🔒</span> Unlock selling';button.title=!lock.configured?'Set TRADING_UNLOCK_PASSWORD in .env first':lock.unlocked?'Click to lock Sell call and Sell put':'Password required to enable Sell call and Sell put';if(state.lockExpiryTimer){clearTimeout(state.lockExpiryTimer);state.lockExpiryTimer=null}if(lock.unlocked&&lock.expires_at){const delay=Math.max(0,lock.expires_at*1000-Date.now());state.lockExpiryTimer=setTimeout(loadTradingLock,Math.min(delay+250,2147483647))}syncButtons()}
...
async function toggleTradingLock(){if(!state.tradingLock.unlocked){openUnlockModal();return}try{state.tradingLock=await api('/api/trading/lock',{method:'POST',body:'{}'});renderTradingLock();toast('Sell call and Sell put are locked.')}catch(error){toast(error.message,true)}}
...
function syncButtons(){const contracts=scenario().contracts||1,unlocked=state.tradingLock.unlocked;$('sellCallBtn').disabled=!unlocked||!state.selectedCall;$('sellPutBtn').disabled=!unlocked||!state.selectedPut;const legs=state.position?.short_legs||[];$('closeCallBtn').disabled=!legs.some(l=>l.type==='call');$('closePutBtn').disabled=!legs.some(l=>l.type==='put');const lockHint=unlocked?`${contracts} contract(s)`:'Unlock selling first';$('sellCallBtn').title=lockHint;$('sellPutBtn').title=lockHint;}
...
function openOrder(order){if(!order.limit_price){toast('No valid midpoint is available for this contract.',true);return}state.pendingOrder=order;$('modalTitle').textContent=order.label;$('orderTicket').innerHTML=`<div><span>Account</span><strong>${document.body.dataset.paper==='true'?'PAPER':'LIVE'}</strong></div><div><span>Contract</span><strong>${order.symbol}</strong></div><div><span>Intent</span><strong>${order.position_intent.replaceAll('_',' ')}</strong></div><div><span>Quantity</span><strong>${order.qty}</strong></div><div><span>DAY limit</span><strong>${money(order.limit_price,2)}</strong></div>`;$('confirmOrder').className=`action-btn ${order.position_intent==='sell_to_open'?'call':'put'}`;$('orderModal').classList.remove('hidden');}
function closeModal(){$('orderModal').classList.add('hidden');state.pendingOrder=null}
async function submitOrder(){if(!state.pendingOrder)return;$('confirmOrder').disabled=true;$('confirmOrder').textContent='Submitting…';try{const data=await api('/api/orders',{method:'POST',body:JSON.stringify({...state.pendingOrder,confirm:true})});toast(`${data.paper?'Paper':'Live'} order accepted: ${data.status}`);closeModal();await loadPortfolio(true)}catch(error){toast(error.message,true);await loadTradingLock()}finally{$('confirmOrder').disabled=false;$('confirmOrder').textContent='Submit order'}}
