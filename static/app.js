...
function renderTrades(trades) {
  ...
  $('tradesBody').innerHTML = rows.length ? rows.map(t => `<tr><td>${t.time?new Date(t.time).toLocaleDateString():'—'}</td><td><strong>${t.underlying}</strong></td><td><span class="mono">${t.symbol}</span><br><span style="color:var(--muted)">${t.strategy}</span></td><td>${t.side}</td><td class="mono">${num(t.qty,0)}</td><td class="mono">${money(t.price,2)}</td><td><span class="status ${t.status}">${t.status}</span></td><td>${t.cancelable&&t.order_id?`<button class="cancel-order-btn" data-order-id="${t.order_id}" data-symbol="${t.symbol}" ${state.tradingLock.unlocked?'':'disabled'}>${state.tradingLock.unlocked?'Cancel order':'🔒 Cancel order'}</button>`:'—'}</td></tr>`).join('') : '<tr><td colspan="8" class="empty-cell">No matching trades</td></tr>';
